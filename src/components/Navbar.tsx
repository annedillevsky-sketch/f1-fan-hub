...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const navItems = [
    { id: 'timing', label: 'Live Timing', icon: Activity },
    { id: 'teams', label: 'Teams & 3D Cars', icon: Car },
    { id: 'circuit', label: 'Circuit Map', icon: Map },
    { id: 'telemetry', label: 'Telemetry Studio', icon: LineChart },
    { id: 'strategy', label: 'Pit Strategy', icon: Timer },
    { id: 'standings', label: 'Standings', icon: Trophy },
    { id: 'calendar', label: 'Calendar & Alerts', icon: Calendar },
    { id: 'mydrivers', label: 'My Drivers', icon: Star, badge: favoriteDriversCount > 0 ? favoriteDriversCount : null },
    { id: 'news', label: 'News & History', icon: BookOpen },
  ];
  ...
  return (
    <header className={`sticky top-0 z-40 border-b transition-colors duration-200 ${
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <span className="font-racing font-extrabold text-lg sm:text-xl tracking-wider text-slate-100">
                  APEX<span className="text-red-500">.LIVE</span>
                </span>
                <span className="px-1.5 py-0.2 bg-red-600/20 text-red-400 border border-red-500/30 rounded text-[9px] font-mono font-bold uppercase tracking-wider">
                  2026 PRO
                </span>
              </div>
              <p className="text-[10px] text-slate-400 tracking-tight hidden sm:block">Official Telemetry & Timing Hub</p>
            </div>
          </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold tracking-wide flex items-center gap-1.5 transition-all relative ${
                    isActive
                      ? isDarkMode
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {user ? (
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-red-500/40 bg-red-600 text-white font-semibold text-xs shadow-md shadow-red-600/20 hover:bg-red-700 transition"
            >
      ...
      <div className="xl:hidden border-t overflow-x-auto no-scrollbar py-2 px-3 flex items-center gap-1">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
